...
<head>
<meta charset="UTF-8">
<title>스프링 MVC 게시판 내용 보기와 아작스 댓글 프로그램 결합</title>
...
<tr>
	...
	'/controller/board/board_cont?bno=${b.bno}&page=${page}&state=edit';">
	<%--state=edit 구분값 때문에 동일 매핑 주소를 사용해도 수정폼으로 이동한다. --%>
	
	
	<c:if test="${b.replycnt==0}">
	<input type="button" value="삭제" onclick="if(confirm('정말로 삭제할까요?')==true){
		location='/controller/board/board_del?bno=${b.bno}&page=${page}';}else{retrun;}">
	<%--자바 스크립트에서 confirm() 내장함수는 확인/취소 버튼을 가진 경고창을 만들어 준다. 
	확인을 클릭하면 true를 반환하고 취소를 클릭하면 false를 반환한다.
	즉 확인을 클릭하면 삭제로 이동하고 취소를 클릭하면 return;에 의해서
	종료가 되어서 현재 내용보기에 그대로 있는다. 다시 한 번 더 삭제 유무를
	확인하게 한다. --%>
	</c:if>
...
[<strong>${b.replycnt}</strong>개의 댓글]  <!-- 댓글 개수  -->
	...
	$('#replyAddBtn').on('click',function(){
		...
		$.ajax({
			...
			success:function($data){
				if($data=='SUCCESS'){
					alert('댓글이 등록되었습니다!');
					location.reload(); // ★ 자바스크립트에서 새로고침(단축키는 F5)
