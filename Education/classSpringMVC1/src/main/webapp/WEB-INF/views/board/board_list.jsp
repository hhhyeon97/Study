...
<table border="1">
	...
	<tr>
		...
		</td>							<%--EL에 사용한 totalCount는 BoardController.java에서 사용한 키이름이다. --%>
	</tr>
	...
	<c:if test="${!empty blist}"> <!-- 목록이 비어있지 않다면! -->
		<c:forEach var="b" items="${blist}">
		<tr>
			<th>${b.bno}</th> <%-- b.bno는 b.getBno();와 같은 기능이다. --%>
			<th>
			<a href="/controller/board/board_cont?bno=${b.bno}&page=${page}&state=cont">${b.title}</a>
			<%--board_cont?bno=번호&page=쪽번호가 get방식으로 bno에 번호,page에 쪽번호,
				state=cont가 &구분기호로 구분해서
				3개의 피라미터값이 get방식으로 전달된다. 
...
<SCRIPT>
	$msg="${result}"; //자바스크립트에서 jsp문법인 EL(표현언어)이나 JSTL 사용가능함.
	//BoardController.java에 result 키이름에 저장된 success문자를 EL인 ${result} 가져온다.
