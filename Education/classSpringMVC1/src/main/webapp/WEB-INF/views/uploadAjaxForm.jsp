...
$(document).ready(function(){
	$('#uploadBtn').on('click',function(e){
		...
		/* 첨부 파일을 업로드 하는 또 다른 방식은 비동기식 아작스를 이용해서 폼데이터 객체를 이용하는 것이다.
		이 방식은 IE 10버전이후에서만 지원된다. */
		
		var inputFile = $("input[name='uploadFile']"); // 아이디 선택자 말고 네임피라미터 이름으로 
		// jQuery에서  input type="file"에 접근한다.
		
		var files = inputFile[0].files; //첫번째 input type="file"로 첨부한 것을 배열로 구함.
		
		for(var i=0;i<files.length;i++){
			formData.append("uploadFile", files[i]); //첨부 파일을 폼데이터에 추가 
		}
		$.ajax({
