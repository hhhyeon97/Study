...
public class UploadController {  // 파일 첨부 즉 이진파일 업로드 스프링 컨트롤러 클래스 
	...
	public void uploadForm() {
		//리턴 타입이 없는 void형이면 매핑주소가 뷰페이지 파일명이 된다. - > 뷰리졸브 경로는 /WEB-INF/views/uploadForm.jsp
	} //uploadForm()
	...
	public void uploadFormAction(MultipartFile[] uploadFile,HttpServletRequest request) {
		/*
		 * MultipartFile 스프링 api를 사용해서 업로드 되는 파일 데이터를 쉽게 처리, 다중 업로드 파일은 배열로 받는다.
		 * 매개변수명 (즉 전달인자명) uploadFile은 <input type="file" name="uploadFile"
		 * 네임피라미터 이름(네임속성)과 같아야 한다.
		 */
		String uploadFolder = request.getRealPath("/resources/upload"); // 첨부 파일 업로드 실제 경로를 구함. 
		...
		for(MultipartFile multi:uploadFile) {
			...
			try {
				multi.transferTo(saveFile); //업로드 폴더에 첨부파일 실제 원본파일명으로 업로드 됨.
			}catch(Exception e) {
				e.printStackTrace();}
		}//for
		
		/*	문제 ) Tomcat 서버에 의해서 실제 upload 폴더에 첨부된 파일이 업로드 되게 만들어 보자. 1개 첨부 파일뿐만 아니라
		 * 		다중 파일 첨부확인도 해보자. 에러가 나면 디버깅도 해보고 정상적인 프로그램이 될 때 까지 개발자 테스트 과정을 해보자. 
		 * 
		 */
	...
	public void uploadAjaxAction(MultipartFile[] uploadFile) {
		...
		for(MultipartFile multi:uploadFile) {
			...
			/*
			 * IE인 경우는 전체 파일경로가 전송되기 떄문에 마지막 경로 부분 \이후부터 마지막 문자까지 구한다. 즉 첨부된 실제 파일명만 구한다.
			 */
			...
			try {
				multi.transferTo(saveFile); // 실제 첨부파일을 업로드 (c드라이브 upload2)
			}catch(Exception e) {e.printStackTrace();}
			/*	문제 ) 비동기식으로 1개나 하나 이상 다중 첨부 파일이 C:\\upload2폴더에 
			 * 실제 업로드가 되는지 확인해보고 에러가 난 경우는 디버깅하면서 개발자 단위 테스트까지 해보자.
			 */
			
			/*
			 * java.lang.StringIndexOutOfBoundsException: String index out of range: -1 오류 발생 
			 * 				\\는 자바 이스케이프 문자 때문에 실제는 \인 것 ! 
			 * lastIndexOf("\\")+1) 에 1 추가해서 해결 -> 마지막 경로 \ 이후부터의 값만 가져오자는 의미 
			 */
