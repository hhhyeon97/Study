...
@Repository // @Repository 애노테이션을 등록함으로써 스프링에 모델 DAO로 인식되게 한다.
public class BoardDAOImpl implements BoardDAO {
	...
	public void insertBoard(BoardVO b) {
		...
		//this. 은 생략 가능함. mybatis에서 insert()메서드는 레코드를 저장한다. board_in은 board.xml에서
		//설정할 유일 아이디명이다. 
	}// 스프링 MVC 게시판 저장
	
	@Override
	public int getTotalCount() {
		return sqlSession.selectOne("board_count"); // mybatis에서 selectOne()메서드는 단 한 개의
	...
	public void updateHit(int bno) {
		sqlSession.update("board_hit",bno); // mybatis에서 update()메서드는 레코드를 수정한다.
	...
	public void updateReplyCnt(int bno, int count) {
		...
		this.sqlSession.update("updateReplyCnt",pm);	// board.xml에서 설정할 유일 아이디명
	}// 새로운 댓글이 추가되면 댓글 개수 1 증가 또는 댓글 하나가 삭제되면 댓글 개수 1 감소 
