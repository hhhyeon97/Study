...
@Setter
@Getter
@ToString(exclude="member") //toString()메서드를 호출했을 때 member변수를 제외 시켜서 호출 안 되게 한다.
@Entity
@SequenceGenerator( // 시퀀스 생성기 
		name="bno_seq3_gename",  // 시퀀스 제너레이터
...
public class Profile {
	...
	private boolean current2; // 현재 사용중인 프로필 사진은 true, 사용하지 않으면 false(0)
	
	@ManyToOne // 다대일 연관관계 
	private MemberVO member; // foreign key( 외래키 - > 주종관계 : Fetch join문을 생성 )
	// 주인테이블인 tbl_members의 uid2 컬럼 회원 아이디값만 저장됨. 
