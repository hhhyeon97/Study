...
@SpringBootTest
@Log // lombok 라이브러리로 로그 기록을 남길 때 이용한다.
...
public class ProfileTests {
	...
	//20명 회원 자료 추가
	//@Test      // @Test에 주석처리하면 test 실행 안 됨 !! 
	public void testInsertMember() {
		IntStream.range(1,21).forEach(i->{ //1부터 20까지의 정수 숫자 생성
			MemberVO member = new MemberVO();
			
			member.setUid2("user"+i);  // 회원 아이디 저장
			member.setUpw("pw"+i);   // 비번 
			member.setUname("사용자"+i); // 회원 이름
			
			//System.out.println("\n===========> tbl_members 테이블 회원 20명 저장하는 JPA");
	...
	public void testInsertProfile() {
		...
		for(int i=1; i<5; i++) {
			...
			profile01.setFname("face"+i+".jpg");  // 4개의 프로필 사진 파일명 저장
			
			if(i==1) {
				profile01.setCurrent2(true); // 첫번째 회원 프로필 사진은 현재 사용중인걸로 처리
			}
	...
	//user1 아이디 정보와 프로필 사진 개수 확인 = > Fetch join01 (연관관계)
	//@Test
	public void testFetchJoin01() {
		System.out.println(" \n====> user1 회원 아이디 정보와 프로필 사진 개수");
	...
	public void testFetchJoin02() {
		...
		System.out.println(" \n=========> user1 회원 정보와 현재 사용중인 프로필 사진 정보");
